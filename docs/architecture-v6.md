...
| V6.25 | Reasoning Engine | Neue zentrale Komponente: Frame+Context+WorldModel+Capabilities+Constraints → `ResolvedSemanticIntent`, keine Service-Calls | neu, vorbereitend (`nlu/reasoning.py`) – `ReasoningEngine.resolve()` baut aus `SemanticFrame.target`/`.area`/`.property` (mit `ConversationContext.last_area`/`.last_floor` als Fallback für Folgeäußerungen ohne eigenes Ziel) `Constraints` und delegiert die Kandidatenfilterung vollständig an `constraint_resolver.resolve_candidates()` (Regel 6, keine Parallel-Suche). Kein Import aus `service_call.py`/`service_mapper.py`. Noch nicht in `engine.py`/`conversation.py` verdrahtet – wie `ConversationContext`/`SemanticDegree`/`NumericValue` vorbereitend, volle Pipeline-Integration folgt erst in Wave 7 |
