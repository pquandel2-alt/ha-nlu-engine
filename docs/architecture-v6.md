...
Abschnitt 8). Stand: Semantic Compiler 2.0 / HomeIntent 4.29.0
(spanbasiertes Lexikon, gemeinsamer Query-/Command-Analysepass,
World-Model-Indizes und gemeinsamer Automation-State-Predicate-Compiler),
2026-08-22.
...
Response/Debug. Die V6.28/V6.29-Zeilen in Abschnitt 6 wurden auf den
aktuellen produktiven Stand nachgezogen.
...
| World Model Indexes | live | `WorldModel` indiziert Domäne, Geräteklasse, Raum, Etage und Capability; der semantische Compiler nutzt diese Indizes live |
| Capability Reasoning | live, erweiterbar | konkrete Fähigkeiten für Kern- und Zusatzdomänen einschließlich Kalender-Mutationen |
...
| Direction / Degree / Condition | teilweise live | typisierte Richtung und Gradsemantik; Bedingungen im AutomationModel |
| Semantic Lexicon | live | `nlu/semantic_lexicon.py` liefert typisierte Kandidaten mit Quellspannen, Konflikten und unerklärten Tokens; Hassil-Slotlisten bleiben kompatibler Altpfad |
| Semantic Composer | teilweise live | `nlu/semantic_compiler.py` kompiliert direkte Gerätebefehle sowie plurale State-/Existenz-/Anzahl-/Listen-/Ortsfragen kompositionell |
| World Model (vereinheitlicht) | live | `world_model.py`, wird pro Conversation-Turn aufgebaut |
| Reasoning Engine | verhaltenswirksam live | `nlu/reasoning.py`; explizite Einzelziele passieren vor Ausführung ein zentrales Konsistenz-Gate |
| ResolvedSemanticIntent | existiert | `nlu/reasoning.py::ResolvedSemanticIntent` |
...
| V6.3 | Semantic Lexicon | Neue Komponente: Wort/Ausdruck → semantische Kandidaten (liefert Kandidaten, entscheidet nicht allein) | **live** (`nlu/semantic_lexicon.py`: dependency-freie Kandidaten mit Textspannen, Konflikterkennung und Coverage; `nlu/lexicon.py`: kompatible Hassil-Slotdaten) |
| V6.4 | Semantic Composer | Setzt unabhängig erkannte Bedeutungsbausteine zu einer semantischen Struktur zusammen statt vollständige Sätze aufzuzählen | **live** (`nlu/semantic_compiler.py` als sicherer Laufzeit-Fallback für direkte Geräte-, Gruppen- und Prozentbefehle sowie plurale Zustandsabfragen; `nlu/composer.py` bleibt der kleine primitive Vorläufer). Aktion beziehungsweise Frageoperation, Ziel, Domäne/Geräteklasse, Zustand, Ort/Etage, Anzahl und Prozentwert werden frei kombiniert, gegen aktuelle HA-Entitäten/Capabilities geprüft und bei Konflikt oder Ambiguität abgelehnt. Derselbe Action-Pfad wird über die Bridge auch von relativen und triggerbasierten Automationen verwendet. |
| V6.5 | Semantische Unvollständigkeit | `SemanticFrame` darf unvollständig sein; Auflösung über Context → explizites Target → Area → World Model → Capability → Default Scope, sonst AMBIGUOUS/MISSING_TARGET | **live**: `semantic_dialog.py` hält genau eine fehlende Auswahl, ein Ziel oder einen Wert typisiert im `ConversationContext`; zufällige Mengenauswahl und unbeschränkte Text-History bleiben ausgeschlossen |
| V6.6 | Home World Model | Entity-Modell zu semantischer HA-Sicht erweitern (Entities, Devices, Areas, Floors, Domains, Device Classes, Capabilities, States, Attributes, Aliases, Relationships); HA bleibt Quelle der Wahrheit | **live** (`WorldModel` wird einmal pro Conversation-Turn aufgebaut und in den Parser-Kontext gegeben) |
| V6.7 | World Model Indexes | Performance-Indizes (domain, device_class, area, floor, capability, alias); States nicht unkontrolliert dauerhaft cachen | **live** für domain, device_class, area, floor und capability (`WorldModel.select_entities()`); Namens-/Aliasindex bleibt in `EntityIndex`; Live-State wird bewusst nicht gecacht |
...
| V6.14 | Quantity Semantics | Internes `QuantityConstraint`-Modell (alle/beide/drei/ein paar/alle außer...) statt Satzregeln | **live**: ALL/EXACTLY und geprüfte EXCLUDE-Ziele laufen über `SemanticQuantity`; SOME startet eine konkrete Auswahlliste statt zufällig Geräte zu wählen. Mehrere koordinierte Orte und Ausschlüsse werden gemeinsam aufgelöst. |
| V6.15 | Degree Semantics | "etwas/leicht/deutlich/viel/komplett" → SLIGHT/MODERATE/STRONG/MAXIMUM/MINIMUM | **live**: `nlu/degree_semantics.py` normalisiert leichte, mittlere und starke relative Schritte für Licht und Solltemperatur; absolute Endlagen bleiben über die Positionssemantik abgedeckt. |
| V6.16 | Number Normalization | "50/50%/fünfzig Prozent" etc. → einheitliches `NumericValue` | **live**: `SemanticFrame` überführt die bereits normalisierten Legacy-Parameter für Prozent, Celsius und Stufe automatisch in ein typisiertes `NumericValue`. Die alten Parameter bleiben während der Mapper-Migration kompatibel erhalten. |
| V6.17 | Comparison Semantics | "mehr als/mindestens/höchstens" → `Comparison(operator, value)`, generisch statt an Prozent/Temperatur gekoppelt | **live im gemeinsamen Lexikon** (`SemanticKind.COMPARATOR` → gt/lt/gte/lte). `ComparisonQueryParser` kombiniert Ziel, Ort, Operator, Wert und Einheit frei und prüft die fachliche Domain-/Einheitenpaarung |
| V6.18 | Query Semantics | Queries und Commands nutzen dieselben Bausteine, Unterschied nur in der Operation | **teilweise live**: `SemanticQueryCompiler` und `SemanticCommandCompiler` konsumieren dasselbe `SemanticAnalysis`; strukturierte `QueryCommand`/`QueryResult` bleiben absichtlich vom schreibenden `SemanticCommand` getrennt |
...
| V6.20 | Natural Query Language | Verschiedene Frageformen ("sind offen?"/"noch offen?"/"gibt es offene...?") auf dieselbe Semantik abbilden | **live und in Migration**: Der semantische Compiler kombiniert Geräteklasse, Zustand, Ort/Etage und Query-Operation unabhängig von ihrer Reihenfolge. Unterstützt State, Exists, Count, List, All/None und Locations. Messwert- und Vergleichsfragen kombinieren zusätzlich Property, Ort, Comparator, Schwellenwert und Einheit frei; spezialisierte Einzelentitätsparser bleiben im Hassil-Kompatibilitätspfad |
| V6.21 | Natural Command Language | Analog für Commands: viele Formulierungen → dieselbe Action/Target/Property-Kombination | **live und in Migration**: `SemanticCommandCompiler` kombiniert Aktion, Zieltyp, Registry-Name, Ort beziehungsweise Etage, Menge, Ausschlüsse und Prozentwert unabhängig von einer festen Wortstellung. Hassil-Grammatiken bleiben als schneller Spezial- und Kompatibilitätspfad erhalten. |
| V6.22 | Multi-Command Semantics | `CommandPlan` mit mehreren Commands, gesamter Plan wird vor Ausführung validiert (nicht Ad-hoc-Split) | **live**: Vor einer Trennung versucht die Engine den gesamten Satz semantisch zu kompilieren, damit „Küche und Flur“ als koordinierter Ort erhalten bleibt. Nur echte unabhängige Teilbefehle werden getrennt; alle Teilpläne müssen vollständig validieren, bevor irgendein Dienst ausgeführt wird. |
...
| V6.25 | Reasoning Engine | Neue zentrale Komponente: Frame+Context+WorldModel+Capabilities+Constraints → `ResolvedSemanticIntent`, keine Service-Calls | `nlu/reasoning.py` baut die Constraints und delegiert an `constraint_resolver.resolve_candidates()`. `engine.py::_build_match_result()` verwendet das Ergebnis für explizit aufgelöste Einzelziele als Ausführungs-Gate: Parserauflösung und zentrale Entity-/Domain-/Area-/Floor-/Capability-Auflösung müssen übereinstimmen. Gruppen- und Mengenlogik bleibt weiterhin in ihren dafür zuständigen Resolvern. |
...
| V6.28 | Automation Foundation | V5-Automationen nutzen dieselben Semantic Frames/Resolver, keine eigene parallele Sprachlogik | **live**: `AutomationModel`, Validator, Generator, Executor, Vorschau/Bestätigung und einmalige Automationen sind produktiv; direkte Action-Bausteine verwenden bereits die semantische Compiler-Bridge |
| V6.29 | Natural-Language Automation | Automation Engine arbeitet vollständig auf semantischer Ebene (Trigger + Action + Property + Value aus natürlicher Sprache) | **live mit klarer Portabilitätsgrenze**: Actions und relative Delayed Commands nutzen gemeinsame Semantik; State-Trigger und State-Conditions teilen `nlu/semantic_automation.py`. Relative, kalendarische, Sonnen-, numerische, Kalender- und Wiederholungsstrukturen werden in das gemeinsame `AutomationModel` übersetzt. Integrationsspezifische Device-Trigger ohne vollständiges HA-Subtypschema werden weiterhin sicher abgelehnt. |
...
kausal-vorsichtiger formulierte Antwort). Das war der historische Scope dieser
Wave. Der aktuelle Stand erweitert ihn um getrennte Trigger-, Condition- und
Action-Referenzen, aktive/deaktivierte Anzahlen, Bereichsfilter und einen
bestätigten, transaktionalen Austausch ausschließlich der Action. Die
folgenden Absätze dokumentieren weiterhin den damaligen Einführungsschritt.
